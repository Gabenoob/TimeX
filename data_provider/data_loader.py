...
class Dataset_Custom(Dataset):
    ...
    def __read_data__(self):
        ...
        cols.remove('date')
        df_raw = df_raw[['date'] + cols + [self.target]]## ？？不知道干了啥
...
class Dataset_M5_(Dataset):
    def __init__(self, args, root_path, flag='train', size=None,
                 features='S', data_path='df_raw.pkl',
                 target='OT', scale=True, timeenc=0, freq='h', seasonal_patterns=None):
        # size [seq_len, label_len, pred_len]
        self.args = args
        # info
...
class Dataset_M5(Dataset):
    def __init__(self, args, root_path, flag='train', size=None,
        ...
        self.date = pd.DataFrame(columns=['date'], data=pd.date_range(start='2011-01-29', periods=1941, freq='D'))

        self.root_path = './concatdata_folder'
        self.data_path = 'HOBBIES_1_008.npy'
        self.calendar = 'cal.npy'
        self.__read_data__()

    def __read_data__(self):
        # self.scaler = StandardScaler()
        data = np.load(os.path.join(self.root_path, self.data_path))
        calendar = np.load(os.path.join(self.root_path, self.calendar))
        '''
        df_raw.columns: ['date', ...(other features), target feature]
        '''
        ...
        for i in range(data.shape[2]):
            if self.scale:
                train_data = data[border1s[0]:border2s[0], :, i]
        ...
        calendar = calendar[border1:border2]

        self.calendar = np.concatenate([data_stamp[:,2].reshape(-1,1),calendar], axis=1)
    ...
    def __getitem__(self, index):
        product = int(index//(len(self.data_x)-self.seq_len-self.pred_len+1))
        index = int(index%(len(self.data_x)-self.seq_len-self.pred_len+1))
        s_begin = index
        s_end = s_begin + self.seq_len
        r_begin = s_end - self.label_len
        r_end = r_begin + self.label_len + self.pred_len
        seq_x = self.data_x[s_begin:s_end,:,product]
        seq_y = self.data_y[r_begin:r_end,:,product]

        calendar = self.calendar[s_begin:s_end]
        seq_x = np.concatenate([calendar,seq_x], axis=1)
        calendar = self.calendar[r_begin:r_end]
        seq_y = np.concatenate([calendar,seq_y], axis=1)
    ...
    def __len__(self):
        return self.data_x.shape[2]*(len(self.data_x) - self.seq_len - self.pred_len + 1)
